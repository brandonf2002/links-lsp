        ...
        `Assoc
          ...
          ; ( "error"
            , match res.error with
              | Some e ->
                `Assoc
                  [ "code", `Int (ErrorCode.to_int e.ErrorCode.code)
  ...
  let pretty_print (msg : t) : string =
    ...
    | Response res ->
      let error_str =
        match res.error with
        | Some err ->
          pretty_print_error_code err.ErrorCode.code
          (* Assuming ErrorCode.error has a field named code of type ErrorCode.t *)
        | None -> "None"
      in
      Printf.sprintf
        "Response {\n  jsonrpc: %s,\n  result: %s,\n  error: %s,\n  id: %s\n}"

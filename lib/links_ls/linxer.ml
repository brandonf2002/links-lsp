...
let prelude_file =
  Settings.(
    option ~default:(Some Linkspath.prelude) "prelude"
...
module Phases = struct
  module Parse = struct
    let run : Context.t -> string -> Sugartypes.program Loader.result = Loader.load
    ...
    let interactive : string -> Context.t -> Sugartypes.sentence Loader.result =
      fun ps1 context ->
      let program, pos_context = Parse.Readline.parse ps1 in
      let context' = Context.{ context with source_code = pos_context } in
      Loader.{ program_ = program; context = context' }
    ;;
  end
  ...
  module Desugar = struct
    let run : Sugartypes.program Loader.result -> Sugartypes.program Frontend.result =
      fun Loader.{ context; program_ } -> Frontend.program context program_
    ;;

    let interactive
      : Sugartypes.sentence Loader.result -> Sugartypes.sentence Frontend.result
      =
      fun Loader.{ context; program_ } -> Frontend.interactive context program_
    ;;
  end
  ...
  let dump_lib : out_channel -> unit =
    ...
    Env.String.iter
      (fun name var ->
        let (datatype : string) =
          Types.string_of_datatype (Env.String.find name Lib.typing_env.Types.var_env)
        in
  ...
  let initialise : unit -> Context.t =
    ...
    let context =
      Context.
        { empty with name_environment = Lib.nenv; typing_environment = Lib.typing_env }
    in
    ...
    let result =
      ...
      let venv = Var.varify_env (Lib.nenv, Lib.typing_env.Types.var_env) in
  ...
  let whole_program
    : Context.t -> string -> Links_core.Sugartypes.program Links_core.Frontend.result
    =
    fun initial_context filename ->
    (* Process source file (and its dependencies. *)
    Parse.run initial_context filename |> Desugar.run
  ;;
  ...
  let evaluate_string
    ...
    Parse.string initial_context source_code |> Desugar.run
  ;;
end
